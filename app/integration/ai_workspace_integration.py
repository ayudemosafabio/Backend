...
class AIWorkspaceIntegration(BaseIntegration):
    ...
    def __init__(self, config: Union[Development, Production]) -> None:
        super().__init__(config.WORKSPACE_URL, config.WORKSPACE_KEY)

    async def list_files(self):
        response = await self._client.get(urljoin(self._base_url, "api/v1/files/"))
    ...
    async def delete_file(self, file_id: str): 
        response = await self._client.delete(urljoin(self._base_url, f"api/v1/files/{file_id}"))
        response.raise_for_status()
        return response.json()
